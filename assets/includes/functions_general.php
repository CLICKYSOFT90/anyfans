...
function Wo_GetArtistFromID($id) {
    global $sqlConnect;
    if (empty($id)) {
        return false;
    }
    $user_id = Wo_Secure($id);
//    dd("SELECT * FROM " . T_Artist_PAGES . " WHERE `id` = '{$id}'");
    $query      = mysqli_query($sqlConnect, "SELECT * FROM " . T_Artist_PAGES . " WHERE `page_id` = '{$id}'");
...
function Wo_GetUserFromID($user_name) {
    global $db;
    if (empty($user_name)) {
        return false;
    }
    $user_id = Wo_Secure($user_name);
    $query      = mysqli_query($sqlConnect2, "SELECT * FROM " . T_USERS . " WHERE `username` = '{$user_name}' OR `email` = '{$user_name}'LIMIT 1 ");
    $fetched_data = mysqli_fetch_assoc($query);
    if ($fetched_data) {
        return $fetched_data;
    }
...
//Redirect to another profile
function Wo_GetUserFromRefEmail($email) {
    global $sqlConnect, $db;
    if (empty($email)) {
        return false;
    }
    $email = Wo_Secure($email);
    $query      = mysqli_query($sqlConnect, "SELECT * FROM " . T_USERS . " WHERE `profile_ref` = '{$email}' LIMIT 1");
    $fetched_data = mysqli_fetch_assoc($query);
    if ($fetched_data) {
        return $fetched_data;
    }
...
//Get Another Profile
function Wo_GetUserAnotherProfile($user_id) {
//    dd($user_id);
    global $sqlConnect, $db;
    if (empty($user_id)) {
        return false;
    ...
    $query1      = mysqli_query($sqlConnect, "SELECT * FROM " . T_USERS . " WHERE `user_id` = '{$user_id}' LIMIT 1");
    $fetched_data1 = mysqli_fetch_assoc($query1);
    $query2      = mysqli_query($sqlConnect, "SELECT * FROM " . T_USERS . " WHERE `profile_ref` = '{$fetched_data1['email']}' LIMIT 1");
    $fetched_data2 = mysqli_fetch_assoc($query2);
...
function getUserAnotherProfile($email){
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $query = mysqli_query($sqlConnect, "SELECT * FROM  " . T_USERS . " WHERE `profile_ref` = '{$email}'");
    $query_sql = mysqli_fetch_assoc($query);
    if ($query_sql) {
        return $query_sql;
    }
...
function getUserFromAnotherDatabase($email){
    global $sqlConnect2, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $query = mysqli_query($sqlConnect2, "SELECT * FROM  " . T_USERS . " WHERE `email` = '{$email}'");
    $query_sql = mysqli_fetch_assoc($query);
    if ($query_sql) {
        return $query_sql;
    }
