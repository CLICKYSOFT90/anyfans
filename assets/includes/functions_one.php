...
//            dd($mysqli_hash_upgrade['password']);
...
function Wo_CreateLoginSession($user_id = 0, $another_user = false) {
    global $sqlConnect, $db, $db2;
    if (empty($user_id)) {
        return false;
    }
    $user_id   = Wo_Secure($user_id);
...
//    $query_two = mysqli_query($sqlConnect2, "DELETE FROM " . T_APP_SESSIONS . " WHERE `session_id` = '{$hash}'");
    if ($query_two) {
        $ua = json_encode(getBrowser());
        $delete_same_session = $db->where('user_id', $user_id)->where('platform_details', $ua)->delete(T_APP_SESSIONS);
//        $delete_same_session2 = $db2->where('user_id', $user_id)->where('platform_details', $ua)->delete(T_APP_SESSIONS);
...
//        $query_three2 = mysqli_query($sqlConnect2, "INSERT INTO " . T_APP_SESSIONS . " (`user_id`, `session_id`, `platform`, `platform_details`, `time`) VALUES('{$another_user['user_id']}', '{$hash}', 'web', '$ua'," . time() . ")");
        if ($query_three) {
            return $hash;
        }
    }
...
function Wo_CheckUserStatus($user_id, $lastseen, $type = '') {
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_RegisterUser($registration_data, $invited = false) {
    global $wo, $sqlConnect, $sqlConnect2;
    ...
    if ($query) {
//        $query2 = mysqli_query($sqlConnect2, "INSERT INTO " . T_USERS . " ({$fields}) VALUES ({$data})");
...
function Wo_UpdateRefUserData($user_id, $update_data, $unverify = false) {
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_UserSug($limit = 20) {
    ...
    $query_one = " SELECT `user_id` FROM " . T_USERS .
        " WHERE `active` = '1' 
        AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') 
        AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') 
        AND `user_id` NOT IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id}) 
        AND `user_id` <> {$user_id}";
    if (isset($limit)) {
...
function Wo_GetMutualFriends($user_id, $type = '', $limit = '', $after_user_id = '', $placement = array()) {
    ...
    $query = "SELECT f1.*
        FROM " . T_FOLLOWERS . " f1 INNER JOIN " . T_FOLLOWERS . " f2
        ON f1.following_id = f2.following_id WHERE f1.follower_id = {$user_id}
        AND f2.follower_id = {$logged_user_id} AND f1.`following_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$logged_user_id}') AND f1.`following_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$logged_user_id}') AND f1.active = 1 GROUP BY following_id ";
