...
function Wo_GetInfluencer($id)
{
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT influncer_pages.page_id, influncer_pages.name, influncer_pages.platform,
                                                influncer_pages.influencer_category, influncer_pages.location,
                                                influncer_pages.username, influncer_pages.about, 
                                                influncer_pages.sources, influncer_pages.date, 
                                                influncer_pages.snapshot, influncer_pages.snapshot_date, 
                                                influncer_pages.logo as 'logo',influncer_pages.logo_date, 
                                                influncer_pages.facebook,influncer_pages.youtube,
                                               influncer_pages.tiktok,influncer_pages.twitter,
                                                influncer_pages.instagram,
                                                influencer_pages_images.name as 'gallery', 
                                                influencer_pages_images.photo_credit as 'photo_credit',
                                                influencer_pages_images.image_capture as 'image_capture',
                                                influencer_pages_images.id as 'gallery_id',
                                                template_categories.name as cat_name,
                                                template_categories.slug,
                                               template_categories.template_type as 'temp_type'
                                                FROM " . T_INFLUNCER_PAGES . " 
                                                LEFT JOIN influencer_pages_images ON
                                                influncer_pages.page_id = influencer_pages_images.influencer_page_id
                                                LEFT JOIN template_categories ON
                                                template_categories.id = influncer_pages.influencer_category
                                                WHERE `page_id` = {$id}");
    $file = [];
    $photo_credit = [];
    $image_capture = [];
    ...
    if (mysqli_num_rows($query)) {
        ...
        $data['gallery_images'] = $file;
        $data['photo_credit'] = $photo_credit;
        $data['image_capture'] = $image_capture;
...
function GetUserPackageLogs($user_id)
{
    ...
    $user_id = Wo_Secure($user_id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM ". T_PACKAGES_LOGS ." WHERE user_id = '{$user_id}' ORDER BY id DESC");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        }
...
function UpdatePackageLogs($logs, $package_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $fields = '`' . implode('`, `', array_keys($logs)) . '`';
    $data = '\'' . implode('\', \'', $logs) . '\'';
    $query_one = "UPDATE " . T_CUSTOM_PAGES . " SET {$fields} WHERE `id` = {$package_id} ";
    $query = mysqli_query($sqlConnect, $query_one);

    return true;
}

function InsertPackageLogs($logs)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $fields = '`' . implode('`, `', array_keys($logs)) . '`';
    $data = '\'' . implode('\', \'', $logs) . '\'';
    $query = mysqli_query($sqlConnect, "INSERT INTO " . T_PACKAGES_LOGS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    }
    return false;
...
function InsertTemplateRanking($ranking_data, $table)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $fields = '`' . implode('`, `', array_keys($ranking_data)) . '`';
    $data = '\'' . implode('\', \'', $ranking_data) . '\'';
...
function InsertArtistRanking($ranking_data)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $fields = '`' . implode('`, `', array_keys($ranking_data)) . '`';
    $data = '\'' . implode('\', \'', $ranking_data) . '\'';
    $query = mysqli_query($sqlConnect, "INSERT INTO " . T_Artist_RANKING . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    }
    return false;
...
function GetSinglePackages($package_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
...
function Wo_UserVotedInfluencer($user_id, $influencer_id, $soft_delete)
{
    $data = array();
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
    ...
    $user_id = Wo_Secure($user_id);
    $influencer_id = Wo_Secure($influencer_id);
    $soft_delete = Wo_Secure($soft_delete);
    $query = mysqli_query($sqlConnect, "SELECT * FROM ". T_INFLUENCER_RANKING ." where user_id = '{$user_id}' and influencer_id = '{$influencer_id}' and soft_delete = {$soft_delete} ORDER BY id DESC");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        }
...
function Wo_GetInfluencerRankings($artist_category, $soft_delete)
{
    $data = array();
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
    ...
    $artist_category = Wo_Secure($artist_category);
    $query = mysqli_query($sqlConnect, "SELECT *,ranks.name as rank_name,
                                                SUM(influencer_points) as total_points
                                                from ". T_INFLUENCER_RANKING ." 
                                                LEFT JOIN ranks ON influencer_ranking.rank_id = ranks.id 
                                                WHERE template_category_id = {$artist_category} 
                                                and soft_delete = {$soft_delete}
                                                GROUP BY influencer_id
                                                ORDER BY total_points DESC,influencer_ranking.id DESC");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        }
...
function Wo_GetArtistRankings($genre_id, $soft_delete)
{
    $data = array();
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
    ...
    $genre_id = Wo_Secure($genre_id);
    $query = mysqli_query($sqlConnect, "SELECT *,ranks.name as rank_name,
                                                SUM(artist_points) as total_points
                                                from ". T_Artist_RANKING ." 
                                                LEFT JOIN ranks ON artist_ranking.rank_id = ranks.id 
...
function Wo_GetTopInfluencer($category_id, $limit)
{
    $data = array();
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
    ...
    $query = mysqli_query($sqlConnect, "SELECT influencer_ranking.*, influncer_pages.name as influencer_name, influncer_pages.logo,
                                            ifnull(SUM(influencer_points),0) as influencer_old_points,
                                            CAST(SUM(CASE WHEN soft_delete = 0 then influencer_points else 0 end) as DECIMAL(10,2)) as influencer_actual_points
                                            FROM ".T_INFLUENCER_RANKING."
                                            JOIN influncer_pages ON influncer_pages.page_id = influencer_ranking.influencer_id
                                            LEFT JOIN template_categories ON template_categories.id = influencer_ranking.template_category_id
                                            WHERE template_categories.id = '{$category_id}' 
                                            GROUP BY influencer_id
                                            ORDER BY influencer_actual_points DESC
                                            LIMIT {$limit}");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
...
function Wo_GetTopArtist($genre_id, $limit)
{
    $data = array();
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
    ...
    $genre_id = Wo_Secure($genre_id);
    $limit = Wo_Secure($limit);
    $query = mysqli_query($sqlConnect, "SELECT artist_ranking.*, artist_pages.name as artist_name, artist_pages.logo,genre.name,
                                            ifnull(SUM(artist_points),0) as artist_old_points,
                                            CAST(SUM(CASE WHEN soft_delete = 0 then artist_points else 0 end) as DECIMAL(10,2)) as artist_actual_points
                                            FROM ".T_Artist_RANKING."
                                            JOIN artist_pages ON artist_pages.page_id = artist_ranking.artist_id
                                            LEFT JOIN genre ON genre.id = artist_ranking.genre_id
                                            WHERE genre_id = '{$genre_id}' 
                                            GROUP BY artist_id
                                            ORDER BY artist_actual_points DESC
                                            LIMIT {$limit}");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
...
function Wo_GetRank($id)
{
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM ". T_RANKS ." WHERE id = '{$id}'");
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        return $fetched_data;
    } else{
...
function Wo_GetArtist($id)
{
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT artist_pages.page_id, artist_pages.name, artist_pages.platform,
                                                artist_pages.about, artist_pages.source,artist_pages.source_date,
                                               artist_pages.artist_category,
                                                artist_pages.location, artist_pages.username,
                                                artist_pages.albums, artist_pages.genre, 
                                                artist_pages.facebook, artist_pages.twitter, artist_pages.tiktok, artist_pages.youtube, artist_pages.instagram, 
                                                artist_pages.logo as 'logo', 
                                                artist_pages.logo_date,
                                                artist_pages_images.name as 'gallery', 
                                                artist_pages_images.photo_credit as 'photo_credit',
                                                artist_pages_images.image_capture as 'image_capture',
                                                artist_pages_images.id as 'gallery_id',
                                                template_categories.name as 'cat_name',
                                                template_categories.template_type as 'temp_type',
                                                genre.name as genre_name,
                                                genre.id as genre_id
                                                FROM " . T_Artist_PAGES . " 
                                                LEFT JOIN artist_pages_images ON
                                                artist_pages.page_id = artist_pages_images.artist_page_id
                                                LEFT JOIN template_categories ON 
                                                artist_pages.artist_category = template_categories.id
                                                LEFT JOIN genre ON 
                                                 artist_pages.platform = genre.id
                                                WHERE `page_id` = {$id}");
    $file = [];
    $photo_credit = [];
    $image_capture = [];
...
function Wo_GetTemplateComments($temp_id, $temp_type)
{
    $data = array();
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
    ...
    $temp_id = Wo_Secure($temp_id);
    $temp_type = Wo_Secure($temp_type);
    $query = mysqli_query($sqlConnect, "SELECT  
                                                template_comments.id,template_comments.user_id, template_comments.template_id, 
                                                template_comments.template_type, template_comments.comment,
                                               template_comments.created_at, template_comments.updated_at,
                                               wo_users.avatar, wo_users.first_name, wo_users.last_name 
                                                FROM ".T_TEMPLATE_COMMENTS." 
                                                LEFT JOIN wo_users ON
                                                wo_users.user_id = template_comments.user_id
                                                WHERE template_id = '{$temp_id}' 
                                                AND template_type = '{$temp_type}'");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        }
...
//
//    $id    = Wo_Secure($id);
//    $query = mysqli_query($sqlConnect, "SELECT team_pages.page_id, team_pages.team_name,
//                                                team_pages.location, team_pages.championship,
//                                                team_pages.history, team_pages.sources,
//                                                team_pages.photo_credit, team_pages.sports, team_pages.category,
//                                                team_pages.period, team_pages.team_logo as 'logo', team_pages_images.name as 'gallery'
//                                                FROM " . T_TEAM_PAGES . " INNER JOIN team_pages_images ON
//                                                team_pages.page_id = team_pages_images.team_page_id
//                                                WHERE `page_id` = {$id}");
//    $file=[];
//    if (mysqli_num_rows($query)) {
//        while ($fetched_data  = mysqli_fetch_assoc($query)){
...
//    return $data;
//}
function GetTemplateCategoriesByType($template_type_category, $platform = '')
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
...
function GetSingleGenreByID($genre_id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $sql = "SELECT * FROM " . T_GENRE . " WHERE id = '{$genre_id}'";
...
function GetSingleCategories($cat_id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $cat_id = Wo_Secure($cat_id);

    $sql = "SELECT * FROM " . T_TEMPLATE_CATEGORY . " WHERE id = '{$cat_id}'";
...
function GetInfluncerPlatforms()
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_INFLUNCER_Platform);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        };
...
function GetInfluncerCategoriesByID($id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_INFLUNCER_CATEGORY . " WHERE platform_id =" . $id);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        };
...
function GetAllInfluncerCategories()
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
...
function GetInfluncerCategoriesByPlatformID($platform_id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_TEMPLATE_CATEGORY . " WHERE platform_id = '{$platform_id}'");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = $fetched_data;
        };
...
function GetSingleArtistCategory($id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
...
function GetAllCategoryListing()
{
    ...
    if (mysqli_num_rows($query)) {
        ...
        $array = [];
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            if ($fetched_data['template_type'] != $template_type) {
                $template_type = $fetched_data['template_type'];
            }
...
function GetArtistCategoryListingByCategory($category, $platform)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $query = mysqli_query($sqlConnect, "SELECT ap.*,tc.id as temp_id,
                                               tc.name as cat_name,
                                               tc.template_type as templat_type,
                                               ar.user_id as voted_user,
                                                ar.soft_delete,
                                               gen.name as genre,
                                                ifnull(SUM(ar.artist_points),0) as artist_old_points,
                                                CASt(SUM(CASE WHEN ar.soft_delete = 0 then ar.artist_points else 0 end) as DECIMAL(10,2)) as artist_actual_points
                                                from ".T_Artist_PAGES." as ap                                      
...
function GetUserLastRankToInfluencer($user_id, $influencer_id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $user_id = Wo_Secure($user_id);
    $influencer_id = Wo_Secure($influencer_id);
    $query = mysqli_query($sqlConnect, "SELECT *, ranks.name FROM ". T_INFLUENCER_RANKING .
                                            " LEFT JOIN ranks ON
                                                influencer_ranking.rank_id = ranks.id
                                                WHERE user_id = {$user_id} and influencer_id = {$influencer_id}");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $array = $fetched_data;
        }
...
function GetUserLastRankToArtist($user_id, $artist_id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $user_id = Wo_Secure($user_id);
    $artist_id = Wo_Secure($artist_id);
    $query = mysqli_query($sqlConnect, "SELECT *, ranks.name FROM ". T_Artist_RANKING .
                                            " LEFT JOIN ranks ON
                                                artist_ranking.rank_id = ranks.id
...
function GetArtistRankingInfo($array, $artist_id, $user_id)
{
    $rank_pos = 0;
    foreach ($array as $ranking_info) {
    $rank_pos++;
        if ($ranking_info['artist_id'] == $artist_id) {
            $html = '<div class="curr-ranking">
                        <label>
                            Current Ranking: '. $rank_pos .'
                        </label>
                    </div>
                    <div class="curr-ranking">
                        <label>
                            <a href="javascript:;">
                                Total Ranking Points:'. $ranking_info['total_points'] .'
                                points
                            </a>
                        </label>
                    </div>
...
function GetInfluencerRankingInfo($array, $artist_id, $user_id)
{
    $rank_pos = 0;
    foreach ($array as $ranking_info) {
    $rank_pos++;
        if ($ranking_info['influencer_id'] == $artist_id) {
            $html = '<div class="curr-ranking">
                        <label>
                            Current Ranking: '. $rank_pos .'
                        </label>
...
function GetCategoryTotalPoints($array_artist)
{
    foreach($array_artist as $artist){
        $sum += $artist['artist_actual_points'];
    }
    return $sum;
}

function GetInfluencerCategoryTotalPoints($array_artist)
{
    foreach($array_artist as $artist){
        $sum += $artist['influencer_actual_points'];
    }
    return $sum;
}
...
function GetArtistCategoryListing()
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
...
//    $query = mysqli_query($sqlConnect, "SELECT *, template_categories.name as cat_name,
//                                               template_categories.template_type as templat_type
//                                                FROM ". T_TEMPLATE_CATEGORY.
//                                                " INNER JOIN artist_pages ON
//                                                template_categories.id = artist_pages.artist_category
//                                                order by cat_name");
    $query = mysqli_query($sqlConnect, "SELECT ap.*,tc.id as temp_id,
                                               tc.name as cat_name,
                                               tc.template_type as templat_type,
    ...
    if (mysqli_num_rows($query)) {
        ...
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            if($fetched_data['genre'] != $genre){
                $genre = $fetched_data['genre'];
            }
            $array[$cat_name][$genre][] = $fetched_data;
        }
    }
    return $array;
}
...
//        return false;
//    }
//    $query = mysqli_query($sqlConnect, "SELECT *, influencer_categories.name as cat_name FROM " . T_INFLUNCER_CATEGORY .
//        " INNER JOIN influncer_pages ON
//                                                    influencer_categories.id = influncer_pages.influencer_category order by cat_name");
//    if (mysqli_num_rows($query)) {
//        $cat_name = '';
//        $array = [];
//        while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function GetSingleInfluencerCategories($id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
...
function GetSingleInfluencerPlatform($id)
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_INFLUNCER_Platform . ' where `id`=' . $id);
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
    }
    return $fetched_data['name'];
...
function Wo_RemoveGalleryImage($id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_ARTIST_PAGES_IMAGES . " WHERE `id` = {$id}");
    if ($query) {
        return true;
    }
    return false;
...
function Wo_RemoveInfluencerGalleryImage($id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
...
function Wo_DeleteArtistPage($id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_Artist_PAGES . " WHERE `page_id` = {$id}");
    if ($query) {
        return true;
    }
    return false;
...
function Wo_GetInfluencerCategory($id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
...
function Wo_DeleteinflunecerPage($id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_INFLUNCER_PAGES . " WHERE `page_id` = {$id}");
    if ($query) {
        return true;
    }
    return false;
...
function ImageUpload($file_name, $file_temp_name, $path)
{
    $data['status'] = 400;
    $target_dir = $path;
...
//    }
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($file_temp_name, $target_file)) {
            $data['message'] = htmlspecialchars(basename($file_name));
            $data['status'] = 200;
        } else {
...
function CheckUsernameFromDB($username, $table, $id = '')
{
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $table = Wo_Secure($table);
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . $table . " WHERE username = '{$username}'");
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($fetched_data && $fetched_data['page_id'] !== $id) {
            return true;
        } else {
            return false;
        }
