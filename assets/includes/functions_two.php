...
function Wo_ArtistData($page_id = 0) {
    ...
    $data           = array();
    $page_id        = Wo_Secure($page_id);
    $query_one      = "SELECT * FROM " . T_Artist_PAGES . " WHERE `page_id` = {$page_id}";
    $hashed_page_Id = md5($page_id);
    if ($wo['config']['cacheSystem'] == 1) {
        $fetched_data = $cache->read($hashed_page_Id . '_PAGE_Data.tmp');
        if (empty($fetched_data)) {
...
function Wo_InfluncerData($page_id = 0) {
    global $wo, $sqlConnect, $cache;
    if (empty($page_id) || !is_numeric($page_id) || $page_id < 0) {
        return false;
    }
    $data           = array();
    $page_id        = Wo_Secure($page_id);
    $query_one      = "SELECT * FROM " . T_INFLUNCER_PAGES . " WHERE `page_id` = {$page_id}";
    $hashed_page_Id = md5($page_id);
    if ($wo['config']['cacheSystem'] == 1) {
        $fetched_data = $cache->read($hashed_page_Id . '_PAGE_Data.tmp');
        if (empty($fetched_data)) {
...
function Wo_GetUsersPages($user_id = 0, $limit = 12, $placement = array(), $offset = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
    ...
    $user_id   = Wo_Secure($user_id);
    $query     = " SELECT * FROM " . T_PAGES . " WHERE `user_id` = {$user_id} AND `active` = '1' {$offset_text} ORDER BY `page_id` LIMIT {$limit}";
    if (!empty($placement)) {
        if ($placement['in'] == 'profile_sidebar' && is_array($placement['groups_data'])) {
            foreach ($placement['groups_data'] as $key => $id) {
                $user_data   = Wo_PageData($id);
                if (!empty($user_data)) {
                    $data[]  = $user_data;
                }
            }
...
function Wo_CountUserPages($user_id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
    ...
    $user_id      = Wo_Secure($user_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`page_id`) AS count FROM " . T_PAGES . " WHERE `user_id` = {$user_id} AND `active` = '1' ");
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        return $fetched_data['count'];
    }
...
function Wo_IsPhoneExist($phone) {
    ...
    $query_text   = "SELECT (SELECT COUNT(`user_id`) FROM " . T_USERS . " WHERE `phone_number` = '{$phone}' AND `default_type` = 0) as users";
    $query        = mysqli_query($sqlConnect, $query_text);
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($fetched_data['users'] == 1) {
...
function Wo_GetGenerations(){
    global $wo, $sqlConnect;
    $data         = array();
    $query_one    = "SELECT * FROM " . T_GENERATIONS;
    $sql          = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data[] = $fetched_data;
...
function Wo_CheckGeneration($year){
    global $wo, $sqlConnect;
    $data         = array();
    $query_one    = "SELECT * FROM " . T_GENERATIONS." WHERE ".$year." BETWEEN `birth_start` AND `birth_end`";
    $sql          = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql)) {
        $fetched_data = mysqli_fetch_assoc($sql);
