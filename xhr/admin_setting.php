...
if ($f == 'admin_setting' and (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'add_influencer_page') {
        ...
        if (!empty($_FILES["logo"]["tmp_name"])) {
            ...
            $fileUploaded = ImageUpload($file_name, $file_temp_name, 'upload/influnecer_page/');

            if ($fileUploaded['status'] !== 200 || $fileUploaded['message'] == "Sorry, your file was not uploaded.") {
                $data['status'] = 400;
                $data['message'] = $fileUploaded['message'];
                header("Content-type: application/json");
                echo json_encode($data);
                exit();
            }
        ...
        if (!empty($_FILES["snapshot"]["tmp_name"])) {
            $file_temp_name = $_FILES["snapshot"]["tmp_name"];
            $file_name = $_FILES["snapshot"]["name"];
            $snapshotUploaded = ImageUpload($file_name, $file_temp_name, 'upload/influnecer_page/snapshot/');

            if ($snapshotUploaded['status'] !== 200) {
                $data['status'] = 400;
                $data['message'] = $snapshotUploaded['message'];
            }
        }
        $insert_data = [
            'name' => $_POST['name'],
            'logo_date' => $_POST['logo_date'],
//            'platform' => $_POST['platform'],
            'influencer_category' => $_POST['influencer_category'],
            'location' => $_POST['location'],
            'username' => $_POST['username'],
            'logo' => isset($fileUploaded['status']) == 200 ? $fileUploaded['message'] : '',
            'snapshot' => isset($snapshotUploaded['status']) == 200 ? $snapshotUploaded['message'] : '',
            'snapshot_date' => $_POST['snapshot_date'],
            'about' => $_POST['about'],
            'sources' => $_POST['sources'],
            'date' => $_POST['source_date'],
        ...
        $lastinsertedId = $db->insert(T_INFLUNCER_PAGES, $insert_data);
        $change_date = [
            'ref_user' => $lastinsertedId
        ];
        $db->where('page_id', $lastinsertedId)->update(T_INFLUNCER_PAGES, $change_date);
        if (!empty($_FILES["gallery"])) {
            foreach ($_FILES['gallery']["tmp_name"] as $key => $gallery_files) {
                $file_temp_name = $_FILES["gallery"]["tmp_name"][$key];
                $file_name = $_FILES["gallery"]["name"][$key];
                $galleryUploaded = ImageUpload($file_name, $file_temp_name, 'upload/influnecer_page/gallery/');
                $insert_data_1 = [
                    'influencer_page_id' => $lastinsertedId,
                    'name' => $galleryUploaded['status'] == 200 ? $galleryUploaded['message'] : '',
                    'photo_credit' => $_POST['photo_credit'][$key],
                    'image_capture' => $_POST['image_capture_date'][$key],
                    'upload_by' => $wo['user']['user_id'],
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s'),
                ];
                $db->insert(T_INFLUNCER_PAGES_IMAGES, $insert_data_1);
            }
        }
        $data['status'] = 200;
        $data['message'] = 'Influencer added successfully';
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }

    //Delete Influnecer Page
    if ($s == 'delete_Influnecer_page' && isset($_GET['page_id'])) {
        if (Wo_DeleteinflunecerPage($_GET['page_id']) === true) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    //Get Influnecer category
    if ($s == 'get_influencer_category') {
        foreach (GetInfluncerCategoriesByPlatformID($_GET['platform_id']) as $category) {
            $option .= '<option value="' . $category['id'] . '">' . ucfirst($category['name']) . '</option>';
        }
        header("Content-type: application/json");
    ...
    //Remove Influncer Page
    if ($s == 'remove_influnecer_page') {
        if (!empty($_POST['ids'])) {
            foreach ($_POST['ids'] as $key => $value) {
                if (!empty($value) && is_numeric($value) && $value > 0) {
                    Wo_DeleteinflunecerPage($value);
                }
            }
            $data = ['status' => 200];
            header("Content-type: application/json");
    ...
    //Get Influencer Page
    if ($s == 'get_influencer_page' && isset($_GET['page_id'])) {
        $option = array();
        $data = Wo_GetInfluencer($_GET['page_id']);
        $count = 0;
        $fb = $data['facebook'] == 1 ? "checked" : "";
        $yt = $data['youtube'] == 1 ? "checked" : "";
        $tt = $data['tiktok'] == 1 ? "checked" : "";
        foreach ($data['gallery_images'] as $key => $gallery_images) {
            if (!empty($gallery_images)) {
                $gallery_listing .= '<div class="row remove-' . $data['gallery_id'][$key] . '">
                        <div class="col-md-6" id="normal-query-form">
                            <input type="hidden" value="' . $data['gallery_id'][$key] . '" name="old_gallery_id[]">
                            <div class="form-group form-float">
                                <div class="form-line">
                                    <label class="form-label">Gallery</label>
                                    <input type="file" class="form-control" name="old_gallery[]" accept="image/*">
                                </div>
                                <div class="form-line">
                                    <img src="' . $wo['site_url'] . '/upload/influnecer_page/gallery/' . $gallery_images . '" class="img-fluid d-flex mx-auto" style="margin-top: 10px;">
                                </div>
                            </div>
                        </div>  
                        <div class="col-md-2" id="normal-query-form">
                            <div class="form-group form-float">
                                <div class="form-line">
                                    ...
                                    <input type="text" required class="form-control" value="' . $data['photo_credit'][$key] . '"
                                           name="old_photo_credit[]">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3" id="normal-query-form">
                            <div class="form-group form-float">
                                <div class="form-line">
                                    ...
                                    <input type="date" required class="form-control" value="' . $data['image_capture'][$key] . '"
                                           name="old_image_capture_date[]" placeholder="mm-dd-yy">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-1 text-center">
                            <i class="fa fa-times " title="Click to delete" onclick="DeleteGalleryImage(' . $data['gallery_id'][$key] . ')" style="cursor: pointer;margin-top: 40px;"></i>
                        </div>
                        <div class="col-md-12">
                            <hr>
                        </div>
        ...
        foreach (GetInfluncerPlatforms() as $platform) {
            $selected = $platform['id'] == $data['platform'] ? 'selected' : '';
            $platform_option .= '<option value="' . $platform['id'] . '" ' . $selected . '>' . ucfirst($platform['name']) . '</option>';
        }
        foreach (GetAllInfluncerCategories() as $category) {
            $selected = $category['id'] == $data['influencer_category'] ? 'selected' : '';
            $option .= '<option value="' . $category['id'] . '" ' . $selected . '>' . ucfirst($category['name']) . '</option>';
        }
        $html = '<div class="row">
                    <input type="hidden" value="' . $data['page_id'] . '" name="influencer_id">
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Profile Image</label>
                                <input type="file" class="form-control" name="logo" accept="image/*">
                                <img src="' . $wo['site_url'] . '/upload/influnecer_page/' . $data['logo'] . '" class="img-fluid d-flex mx-auto" style="margin-top: 10px;">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="date" required class="form-control" value="' . $data['logo_date'] . '" name="logo_date">
                            </div>
                        </div>
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Name:</label>
                                <input type="text" required class="form-control" value="' . $data['name'] . '" name="name">
                            </div>
                        </div>
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Insta Handle:</label>
                                <input type="text" required class="form-control" value="' . $data['username'] . '" name="username">
                            </div>
                        </div>
                        <div class="form-group form-float">
                            <div class="form-line">
            ...
            . '</select>
                        ...
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="text" required class="form-control" value="' . $data['location'] . '" name="location">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <textarea required class="form-control" name="about">' . $data['about'] . '</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="text" required class="form-control" name="sources" value="' . $data['sources'] . '">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Date:</label>
                                <input type="date" required class="form-control" value="' . $data['date'] . '" name="source_date">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="file" class="form-control" name="snapshot" accept="image/*">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="date" required class="form-control" value="' . $data['snapshot_date'] . '" name="snapshot_date">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                <img src="' . $wo['site_url'] . '/upload/influnecer_page/snapshot/' . $data['snapshot'] . '" class="img-fluid d-flex mx-auto" style="margin-top: 10px;">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="text" required class="form-control" placeholder="To be generated dynamically" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                    ...
                    <div class="col-md-12 text-right ">
                        <i class="fa fa-plus add-row" data-add="edit" title="Click to add below row" style="cursor: pointer;"></i>
                    </div>
                </div>
                ' . $gallery_listing . '
                <div id="edit"></div>';
...
//    Update Influencer Page
    if ($s == 'update_influencer_page') {
        $data = array();
        if (CheckUsernameFromDB($_POST['username'], T_INFLUNCER_PAGES, $_POST['influencer_id']) == true) {
            $data['status'] = 400;
            $data['message'] = "Username already exist";
            header("Content-type: application/json");
            echo json_encode($data);
        ...
        if (!empty($_FILES["logo"]["tmp_name"])) {
            ...
            $insert_data['logo'] = $fileUploaded['message'];
        ...
        if (!empty($_FILES["snapshot"]["tmp_name"])) {
            ...
            $insert_data['snapshot'] = $snapshotUploaded['message'];
        }
        $lastinsertedId = $db->where('page_id', $_POST['influencer_id'])->update(T_INFLUNCER_PAGES, $insert_data);

        if (!empty($_FILES["old_gallery"])) {
            foreach ($_FILES['old_gallery']["tmp_name"] as $key => $gallery_files) {
                $file_temp_name = $_FILES["old_gallery"]["tmp_name"][$key];
                $file_name = $_FILES["old_gallery"]["name"][$key];
                $galleryUploaded = ImageUpload($file_name, $file_temp_name, 'upload/influnecer_page/gallery/');
                $insert_data_1 = [
                    'influencer_page_id' => $_POST['influencer_id'],
                    'name' => $galleryUploaded['status'] == 200 ? $galleryUploaded['message'] : '',
                    'photo_credit' => $_POST['old_photo_credit'][$key],
                    'image_capture' => $_POST['old_image_capture_date'][$key],
                    'upload_by' => $wo['user']['user_id'],
                    'updated_at' => date('Y-m-d H:i:s'),
                ];
                $db->where('id', $_POST['old_gallery_id'][$key])->update(T_INFLUNCER_PAGES_IMAGES, $insert_data_1);
            }
        }
        if (!empty($_POST["old_photo_credit"]) && !empty($_POST["old_image_capture_date"])) {
            foreach ($_POST["old_photo_credit"] as $key => $old_photo_credit) {
                $insert_data_1 = [
                    'influencer_page_id' => $_POST['influencer_id'],
                    'photo_credit' => $old_photo_credit,
                    'image_capture' => $_POST['old_image_capture_date'][$key],
                    'upload_by' => $wo['user']['user_id'],
                    'updated_at' => date('Y-m-d H:i:s'),
                ];
        ...
        if (!empty($_FILES["gallery"]["tmp_name"])) {
            foreach ($_FILES['gallery']["tmp_name"] as $key => $gallery_files) {
                ...
                $db->insert(T_INFLUNCER_PAGES_IMAGES, $insert_data_2);
            }
        }
        $data['status'] = 200;
        $data['message'] = 'Artist updated successfully';
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }

    if ($s == 'remove_influencer_gallery_image') {
        if (Wo_RemoveInfluencerGalleryImage($_POST['id'])) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    //Add Artist Page
    if ($s == 'add_artist_page') {
        $data = array();
        if ($_POST['artist_category'] == 0) {
            $data['status'] = 400;
        ...
        if ($_POST['platform'] == 0) {
            $data['status'] = 400;
            $data['message'] = "Please Select genre";
            header("Content-type: application/json");
            echo json_encode($data);
            exit();
        }
//        if (CheckUsernameFromDB($_POST['username'], T_Artist_PAGES) == true) {
//            $data['status'] = 400;
//            $data['message'] = "Username already exist";
//            header("Content-type: application/json");
//            echo json_encode($data);
...
//        }
        if (!empty($_FILES["logo"]["tmp_name"])) {
            $file_temp_name = $_FILES["logo"]["tmp_name"];
            $file_name = $_FILES["logo"]["name"];
            $fileUploaded = ImageUpload($file_name, $file_temp_name, 'upload/team_page/');
...
//            'username' => $_POST['username'],
        ...
        $lastinsertedId = $db->insert(T_Artist_PAGES, $insert_data);
        $change_date = [
            'ref_user' => $lastinsertedId
        ];
        $db->where('page_id', $lastinsertedId)->update(T_Artist_PAGES, $change_date);

        if (!empty($_FILES["gallery"])) {
            foreach ($_FILES['gallery']["tmp_name"] as $key => $gallery_files) {
                $file_temp_name = $_FILES["gallery"]["tmp_name"][$key];
                $file_name = $_FILES["gallery"]["name"][$key];
                $galleryUploaded = ImageUpload($file_name, $file_temp_name, 'upload/team_page/gallery/');
                $insert_data_1 = [
                    'artist_page_id' => $lastinsertedId,
                    'name' => $galleryUploaded['status'] == 200 ? $galleryUploaded['message'] : '',
                    'photo_credit' => $_POST['photo_credit'][$key],
                    'image_capture' => $_POST['image_capture_date'][$key],
                    'upload_by' => $wo['user']['user_id'],
                ...
                $db->insert(T_ARTIST_PAGES_IMAGES, $insert_data_1);
            }
        }
        $data['status'] = 200;
        $data['message'] = 'Artist added successfully';
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }
    //Edit Artist Page
    if ($s == 'update_artist_page') {
//        dd($_POST);
        $data = array();
        if ($_POST['artist_category'] == 0) {
            $data['status'] = 400;
        ...
        if (CheckUsernameFromDB($_POST['username'], T_Artist_PAGES, $_POST['artist_id']) == true) {
            $data['status'] = 400;
            $data['message'] = "Username already exist";
            header("Content-type: application/json");
            echo json_encode($data);
...
//            'username' => $_POST['username'],
        ...
        $lastinsertedId = $db->where('page_id', $_POST['artist_id'])->update(T_Artist_PAGES, $insert_data);
        ...
        if (!empty($_FILES["old_gallery"])) {
            foreach ($_FILES['old_gallery']["tmp_name"] as $key => $gallery_files) {
                ...
                $galleryUploaded = ImageUpload($file_name, $file_temp_name, 'upload/team_page/gallery/');
                $insert_data_1 = [
                    'artist_page_id' => $_POST['artist_id'],
                    'name' => $galleryUploaded['status'] == 200 ? $galleryUploaded['message'] : '',
                    'photo_credit' => $_POST['old_photo_credit'][$key],
                    'image_capture' => $_POST['old_image_capture_date'][$key],
                    'upload_by' => $wo['user']['user_id'],
                ...
                $db->where('id', $_POST['old_gallery_id'][$key])->update(T_ARTIST_PAGES_IMAGES, $insert_data_1);
            }
        }
        if (!empty($_POST["old_photo_credit"]) && !empty($_POST["old_image_capture_date"])) {
            foreach ($_POST["old_photo_credit"] as $key => $old_photo_credit) {
        ...
        if (!empty($_FILES["gallery"]["tmp_name"])) {
            foreach ($_FILES['gallery']["tmp_name"] as $key => $gallery_files) {
                ...
                $db->insert(T_ARTIST_PAGES_IMAGES, $insert_data_2);
            }
        }
        $data['status'] = 200;
        $data['message'] = 'Artist updated successfully';
    ...
    if ($s == 'delete_artist_page' && isset($_GET['page_id'])) {
        if (Wo_DeleteArtistPage($_GET['page_id']) === true) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'remove_gallery_image') {
        if (Wo_RemoveGalleryImage($_POST['id'])) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'remove_artist_page') {
        if (!empty($_POST['ids'])) {
            foreach ($_POST['ids'] as $key => $value) {
                if (!empty($value) && is_numeric($value) && $value > 0) {
                    Wo_DeleteArtistPage($value);
                }
            }
            $data = ['status' => 200];
            header("Content-type: application/json");
    ...
    if ($s == 'get_artist_page' && isset($_GET['page_id'])) {
        $option = array();
        $data = Wo_GetArtist($_GET['page_id']);
        $count = 0;
                ...
                $gallery_listing .= '<div class="row remove-' . $data['gallery_id'][$key] . '">
                    <div class="col-md-6" id="normal-query-form">
                        ...
                        <div class="form-group form-float">
                            ...
                            <div class="form-line">
                                <img src="' . $wo['site_url'] . '/upload/team_page/gallery/' . $gallery_images . '" class="img-fluid d-flex mx-auto" style="margin-top: 10px;">
                            </div>
                        </div>
                    </div>  
                    <div class="col-md-2" id="normal-query-form">
        ...
        $option = '<option value="0" disabled>Select Category</option>';
        foreach (GetTemplateCategoriesByType('artist') as $category) {
            $selected = $category['id'] == $data['artist_category'] ? 'selected' : '';
            $option .= '<option value="' . $category['id'] . '" ' . $selected . '>' . ucfirst($category['name']) . '</option>';
        }
        foreach (GetGenre() as $genre) {
            $selected = $genre['id'] == $data['platform'] ? 'selected' : '';
            $genre_option .= '<option value="' . $genre['id'] . '" ' . $selected . '>' . ucfirst($genre['name']) . '</option>';
        }
        $html = '<div class="row">
                    <input type="hidden" value="' . $data['page_id'] . '" name="artist_id">
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Profile Image</label>
                                <input type="file" class="form-control" name="logo" accept="image/*">
                                <img src="' . $wo['site_url'] . '/upload/team_page/' . $data['logo'] . '" class="img-fluid d-flex mx-auto" style="margin-top: 10px;">
                            </div>
                        </div>
            ...
            . '</select>
                        ...
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Location:</label>
                                <input type="text" required class="form-control" value="' . $data['location'] . '" name="location" value="@">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12" id="normal-query-form">
                    ...
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Sources:</label>
                                <input type="text" required class="form-control" value="' . $data['source'] . '" name="source">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                <label class="form-label">Date:</label>
                                <input type="date" required class="form-control" value="' . $data['source_date'] . '" name="source_date">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="text" required class="form-control" value="' . $data['albums'] . '" name="albums">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4" id="normal-query-form">
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <input type="text" required class="form-control" value="' . $data['genre'] . '" name="genre">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6" id="normal-query-form">
                    ...
                    <div class="col-md-12 text-right ">
                        <i class="fa fa-plus add-row" data-add="edit" title="Click to add below row"></i>
                    </div>
                </div>
                ' . $gallery_listing . '
                <div id="edit"></div>';
...
if ($s == 'vote_now') {
    $artist_id = Wo_Secure($_GET['artist_id']);
    $artist_info = Wo_GetArtist($artist_id);
    ...
    if (!empty($artist_info)) {
        ...
        if (!empty($user_voted) && $user_voted['vote_type'] == 0 && $wo['user']['superfan_package'] == 0) {
            ...
            $ranking_data = [
                ...
                'rank_id' => $ranks['id'],
                'artist_points' => $ranks['points'],
                'vote_type' => 0,
                'soft_delete' => 0,
                'package_id' => 0,
                'template_category_id' => $artist_info['artist_category'],
                'genre_id' => $artist_info['platform'],
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s')
            ];
            InsertTemplateRanking($ranking_data, T_Artist_RANKING);
            $data = array(
                'status' => 200,
                'message' => 'You currently have <b>' . $artist_info['name'] . '</b> ranked 1st in this category <br>
                    Sign up to be a SuperFan for $5.99 a month <br>
...
if ($s == 'again_vote_now') {
    $artist_id = Wo_Secure($_POST['artist_id']);
    $artist_info = Wo_GetArtist($artist_id);
    $ranks = Wo_GetRank($_POST['rank']);
    $package = GetSinglePackages(1);
        ...
        if ($user_last_package && $user_last_package[0]['expired_at'] > date('Y-m-d')) {
            if ($user_last_package[0]['votes'] < $facility_package['votes'] || $user_last_package[0]['likes'] < $facility_package['likes']) {
                ...
                $ranking_data = [
                    ...
                    'vote_type' => 1,
                    'soft_delete' => 0,
                    'package_id' => $user_last_package[0]['id'],
                    'template_category_id' => $artist_info['artist_category'],
                    'genre_id' => $artist_info['platform'],
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s')
...
//                        UpdatePackageLogs($update, $user_last_package[0]['id']);
                ...
                $user_updated = Wo_UpdateUserData($wo['user']['user_id'], $Update_data);
                $data = array(
                    'status' => 400,
                    'message' => 'You have consumed your previous package. Please purchase the superfan package.'
                );
                header("Content-type: application/json");
                echo json_encode($data);
                exit();
...
if ($s == 'purchase_package') {
    if ($_GET['type'] == 'superfan') {
        $package = GetSinglePackages(1);
        $facility_package = unserialize($package['facility']);
        if ($wo['user']['wallet'] < $package['amount']) {
            $data = array(
                'status' => 400,
                'message' => 'Notification: In order to buy SuperFan package, your wallet will be charged $5.99/-. <br>Please
                                replenish your wallet if your balance is less than $6.00/- by <a target="_blank" href="">Clicking Here</a>'
            );
            header("Content-type: application/json");
            echo json_encode($data);
            exit();
            ...
            //If User has already purchased package.
            if ($user_package && $user_package[0]['expired_at'] > date('Y-m-d')) {
                //If User had purchased package but did not avail.
                if ($user_package[0]['votes'] < $facility_package['votes'] || $user_package[0]['likes'] < $facility_package['likes']) {
                    $remaining_votes = $facility_package['votes'] - $user_package[0]['votes'];
                    $remaining_likes = $facility_package['likes'] - $user_package[0]['likes'];
                    $data = array(
                        'status' => 400,
                        'message' => 'Please avail your existing package, first!. You have ' . $remaining_votes . ' votes and ' . $remaining_likes . ' likes remaining.'
                    );
                    header("Content-type: application/json");
                    echo json_encode($data);
                    exit();
                    ...
                    $user_updated = Wo_UpdateUserData($wo['user']['user_id'], $Update_data);
                    //If amount deduct from user's wallet.
                    if ($user_updated) {
                        $future_timestamp = strtotime("+1 month");
                        $package_expired_date = date('Y-m-d H:i:s', $future_timestamp);
                        $logs = [
                            'user_id' => $wo['user']['user_id'],
                            'package_id' => 1,
                            ...
                            'created_at' => date('Y-m-d H:i:s'),
                            'updated_at' => date('Y-m-d H:i:s'),
                            'expired_at' => $package_expired_date
                        ];
...
if ($s == 'user_upload_images') {
    if (empty($_POST['photo_credit'])) {
        $data['status'] = 400;
        $data['message'] = "Photo credit field is required";
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }
    if (empty($_POST['credit_date'])) {
        $data['status'] = 400;
        $data['message'] = "Photo credit date field is required";
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }
...
//            if ($user_last_package[0]['image_upload'] < $facility_package['image_upload']) {
        if (!empty($_FILES["user-upload-file"])) {
            ...
            $file_name = $_FILES["user-upload-file"]["name"];
            $fileUploaded = ImageUpload($file_name, $file_temp_name, 'upload/team_page/gallery/');
            if ($fileUploaded['status'] !== 200 || $fileUploaded['message'] == 'Sorry, your file was not uploaded.') {
                $data['status'] = 400;
                $data['message'] = $fileUploaded['message'];
                header("Content-type: application/json");
            ...
            $insert_data = [
                ...
                'image_capture' => $_POST['credit_date'],
                'name' => $fileUploaded['status'] == 200 ? $fileUploaded['message'] : '',
                'upload_by' => $wo['user']['user_id'],
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s'),
            ];
            $db->insert(T_ARTIST_PAGES_IMAGES, $insert_data);
            $update['image_upload'] = $user_last_package[0]['image_upload'] + 1;
            $lastinsertedId = $db->where('id', $user_last_package[0]['id'])->update(T_PACKAGES_LOGS, $update);
            $data = array(
                'status' => 200,
...
//                $data = array(
//                    'status' => 200,
//                    'message' => 'You have consume your image upload limit.'
//                );
//                header("Content-type: application/json");
//                echo json_encode($data);
//                exit();
...
if ($s == 'template_comment') {
    ...
    $artist_info = Wo_GetArtist($_POST['temp_id']);
    if (!empty($artist_info)) {
        $add_data = [
            'user_id' => $wo['user']['user_id'],
                                                ...
                                                <div class="comment-heading">
                                                   <span>
                                                    ...
                                                    <div class="pull-right comment-options comment_edele_options">
                                                      <span class="pointer comment-icons" id="editComment" onclick="Wo_OpenCommentEditBox(6);">
                                                        ...
                                                        <span class="pointer" id="deleteComment" onclick="Wo_DeleteComment(6);">
                                                         <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                              viewBox="0 0 24 24" fill="none"
                                                              stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                              stroke-linejoin="round"
...
if ($s == 'vote_now_influencer') {
    $influencer_id = Wo_Secure($_GET['influencer_id']);
    $influencer_info = Wo_GetInfluencer($influencer_id);
    $ranks = Wo_GetRank($_GET['vote_id']);
    if (!empty($influencer_info)) {
        ...
        if (!empty($user_voted) && $user_voted['vote_type'] == 0 && $wo['user']['superfan_package'] == 0) {
            ...
            $ranking_data = [
                'user_id' => $wo['user']['user_id'],
                'influencer_id' => $influencer_info['page_id'],
                'rank_id' => $ranks['id'],
                'influencer_points' => $ranks['points'],
                'vote_type' => 0,
                'soft_delete' => 0,
                'package_id' => 0,
                'template_category_id' => $influencer_info['influencer_category'],
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s')
            ];
...
if ($s == 'again_vote_now_influencer') {
    $influencer_id = Wo_Secure($_POST['influencer_id']);
    $influencer_info = Wo_GetInfluencer($influencer_id);
    $ranks = Wo_GetRank($_POST['rank']);
    $package = GetSinglePackages(1);
    $facility_package = unserialize($package['facility']);
...
if ($s == 'user_upload_images_influencer') {
    if (empty($_POST['photo_credit'])) {
        $data['status'] = 400;
        $data['message'] = "Photo credit field is required";
        header("Content-type: application/json");
...
//            if ($user_last_package[0]['image_upload'] < $facility_package['image_upload']) {
        if (!empty($_FILES["user-upload-file"])) {
            $file_temp_name = $_FILES["user-upload-file"]["tmp_name"];
            $file_name = $_FILES["user-upload-file"]["name"];
            $fileUploaded = ImageUpload($file_name, $file_temp_name, 'upload/influnecer_page/gallery/');
            if ($fileUploaded['status'] !== 200 || $fileUploaded['message'] == 'Sorry, your file was not uploaded.') {
                $data['status'] = 400;
                $data['message'] = $fileUploaded['message'];
                header("Content-type: application/json");
            ...
            $db->insert(T_INFLUNCER_PAGES_IMAGES, $insert_data);
            $update['image_upload'] = $user_last_package[0]['image_upload'] + 1;
            $lastinsertedId = $db->where('id', $user_last_package[0]['id'])->update(T_PACKAGES_LOGS, $update);
            $data = array(
                'status' => 200,
