...
if ($f == 'login') {
    ...
    if (isset($_POST['username']) && isset($_POST['password'])) {
        ...
        if (empty($errors) && $phone == 0) {
            $another_user = Wo_GetUserFromID($_POST['username']);
            ...
            $session             = Wo_CreateLoginSession(Wo_UserIdForLogin($username), $another_user);
...
//                $data['location'] = $wo['config']['site_url'];
                $data['location'] = $wo['config']['site_url'].'/influencer';
            }
